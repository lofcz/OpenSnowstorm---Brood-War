...
struct state_functions {
	...
	bool player_position_is_visible(int owner, xy position) const {
		if (st.cheat_black_sheep_wall) return true;
	...
	void order_CastDefensiveMatrix(unit_t* u) {
		...
		if (spell_cast_target_movement(u, tech, unit_sight_range(u, true))) {
			if (!st.cheat_the_gathering || u->owner >= 8) u->energy -= fp8::integer(tech->energy_cost);
			unit_t* target = u->order_target.unit;
	...
	void order_Spell(unit_t* u) {
		...
		if (is_in_range()) {
			...
			if (u->spell_cooldown == 0 && unit_can_fire_weapon(u, weapon) && !u_movement_flag(u, 2) && unit_is_at_move_target(u)) {
				if (tech && (!st.cheat_the_gathering || u->owner >= 8)) u->energy -= fp8::integer(tech->energy_cost);
	...
	void order_CastRecall(unit_t* u) {
		if (u->order_state == 0) {
			...
			if (!st.cheat_the_gathering || u->owner >= 8) u->energy -= fp8::integer(get_tech_type(TechTypes::Recall)->energy_cost);
	...
	void execute_main_order(unit_t* u) {
		...
		case Orders::ComputerAI:
			// Computer AI main order: behave like AttackMove toward set target
			order_AttackMove(u);
			break;
		case Orders::AtkMoveEP:
			// Attack move to escort point
			order_AttackMove(u);
			break;
		case Orders::HarassMove:
			// AI harass: move toward target and attack anything in range
			order_AttackMove(u);
			break;
		case Orders::AIPatrol:
			// AI Patrol: use the same logic as the player-facing patrol order.
			order_Patrol(u);
			break;
		case Orders::GuardPost:
			// Guard Post: like Hold Position but AI-controlled; reuses tower logic.
			order_TowerAttack(u);
			break;
		case Orders::RescuePassive:
			// Rescue passive: neutral unit that can be turned friendly.
			order_PlayerGuard(u);
			break;
		case Orders::Neutral:
			order_Neutral(u);
		...
		case Orders::ComputerReturn:
			// Computer units return to their base/rally point.
			if (unit_is_at_move_target(u)) {
				set_unit_order(u, get_order_type(Orders::ComputerAI));
			}
			break;
		...
		case Orders::HiddenGun:
			// Hidden gun emplacement - attack enemies in range, otherwise hold
			order_TowerGuard(u);
			break;
	...
	xy get_image_screen_pos(const image_t* image) const {
		return image->sprite->position + image->offset;
	}

	thingy_t* create_thingy_at_image(image_t* image, const sprite_type_t* sprite_type, xy pos, int elevation_level) {
		xy screen_pos = get_image_screen_pos(image);
		if (i_flag(image, image_t::flag_horizontally_flipped)) pos.x = -pos.x;
		thingy_t* t = create_thingy(sprite_type, screen_pos + pos, image->sprite->owner);
		if (t) {
	...
	void on_hit_aoe_change_target(unit_t* u, unit_t* target) {
		...
		if (on_hit_should_change_target(u, u->auto_target_unit, target)) {
			u->auto_target_unit = target;
			unit_t* turret = unit_turret(u);
			if (turret) on_hit_change_target(turret, target);
			on_hit_change_target(u, target);
		}
	}
	...
	thingy_t* create_thingy(const sprite_type_t* sprite_type, xy pos, int owner) {
		if (!sprite_type) error("attempt to create thingy of null type");
		thingy_t* thingy = st.free_thingies.empty() ? nullptr : &st.free_thingies.front();
		if (!thingy) {
			st.thingies.emplace_front();
			thingy = &st.thingies.front();
		} else {
			st.free_thingies.pop_front();
		}
		thingy->sprite = create_sprite(sprite_type, pos, owner);
		if (!thingy->sprite) {
			st.free_thingies.push_front(*thingy);
			return nullptr;
		}
		st.active_thingies.push_front(*thingy);
		++st.active_thingies_size;
		return thingy;
	}
	...
	void load_map_data(uint8_t* data, size_t data_size, std::function<void()> setup_f = {}, bool initial_processing = true) {
		...
		if (setup_info.resource_type == 1) {
			for (size_t i = 0; i != 12; ++i) {
				...
				st.current_gas[i] = setup_info.starting_gas;
				st.total_gas_gathered[i] = setup_info.starting_gas;
			}
		}
		for (size_t i = 0; i != 12; ++i) {
			if (st.players[i].controller != player_t::controller_occupied) continue;
